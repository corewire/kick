---
title: Running without GitOps
weight: 45
description: KICK does not require Argo CD, Flux or Kargo. GitOps gating is opt-in.
llmsDescription: |
  KickPolicy spec.gitOps.provider defaults to None, so KICK restarts workloads
  on a plain Kubernetes cluster with no GitOps controller installed. Auto
  detection blocks when no provider recognises the workload, so Auto must not
  be used on a cluster without Argo CD, Flux or Kargo. Kargo is never
  auto-detected and must be selected explicitly.
---
