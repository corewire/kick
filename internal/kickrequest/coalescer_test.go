...
func TestEnsureActiveRequestCoalescesRepeatedEvents(t *testing.T) {
	...
	if _, err := coalescer.EnsureActiveRequest(ctx, "payments", target, "policy-a", firstAt); err != nil {
		t.Fatalf("first ensure: %v", err)
	}
	if _, err := coalescer.EnsureActiveRequest(ctx, "payments", target, "policy-a", secondAt); err != nil {
